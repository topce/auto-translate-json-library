...
export class CsvHandler implements IFormatHandler {
  ...
  serialize(data: EnhancedTranslationFile, options?: CsvOptions): string {
    try {
      ...
      const csvOptions = this.getDialectOptions(options?.dialect, {
        ..._metadata?.csvOptions,
        ...options,
      });
      ...
      throw new Error(
        `Failed to serialize CSV: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private detectDelimiter(content: string): string {
    ...
    for (const delimiter of delimiters) {
      ...
      const fieldCounts = lines.map(
        (line) => this.parseCSVLine(line, delimiter).length,
      );
      const firstCount = fieldCounts[0];
      const consistency =
        fieldCounts.filter((count) => count === firstCount).length /
        fieldCounts.length;
  ...
  private parseCSVContent(
    ...
    const hasHeaders =
      options?.hasHeaders !== false
        ? this.detectHeaders(lines, delimiter, options)
        : false;
    let headers: string[];
  ...
  private looksLikeTranslatableText(text: string): boolean {
    ...
    const hasCommonWords = /\b(the|and|or|in|on|at|to|for|of|with|by)\b/i.test(
      text,
    );
    ...
    return (
      (hasSpaces && hasMultipleWords) ||
      hasCommonWords ||
      (isLongEnough && hasVariousChars)
    );
  }
  ...
  private validateDelimiterConsistency(
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (fieldCount !== expectedFieldCount) {
        inconsistentLines++;
        if (inconsistentLines <= 3) {
          // Report first 3 inconsistencies
          warnings.push({
    ...
    if (inconsistentLines > lines.length * 0.3) {
      // More than 30% inconsistent
      errors.push({
  ...
  private processRows(
    ...
    if (!keyColumn) {
      throw new Error(
        "Could not detect key column in CSV. Please specify keyColumn option.",
      );
    }

    const valueColumn =
      options?.valueColumn || this.detectValueColumn(columns, keyColumn);
    if (!valueColumn) {
      throw new Error(
        "Could not detect value column in CSV. Please specify valueColumn option.",
      );
    }

    // Use provided multi-language columns or auto-detect
    const multiLanguageColumns =
      options?.multiLanguageColumns ||
      this.detectMultiLanguageColumns(columns, keyColumn);
    ...
    for (const row of rows) {
      ...
      for (const [langCode, columnName] of Object.entries(
  ...
  private detectMultiLanguageColumns(
    columns: string[],
    keyColumn: string,
  ...
  private detectValueColumn(
    ...
    for (const pattern of valuePatterns) {
      const match = columns.find(
        (col) => pattern.test(col) && col !== keyColumn,
      );
      if (match) return match;
    }
  ...
  private determineColumns(
    ...
    const valueColumn =
      options?.valueColumn || metadata?.valueColumn || "value";
    ...
    const multiLanguageColumns =
      options?.multiLanguageColumns || metadata?.multiLanguageColumns || {};
    for (const [langCode, columnName] of Object.entries(multiLanguageColumns)) {
      if (
  ...
  private escapeField(
    ...
    const needsQuoting =
      ...
      field.startsWith(" ") ||
      field.endsWith(" "); // Leading/trailing spaces

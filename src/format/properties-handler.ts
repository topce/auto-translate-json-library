...
export class PropertiesHandler implements IFormatHandler {
  ...
  private detectEncoding(content: string): string {
    ...
    if (encodingComment) {
      ...
      if (
        declaredEncoding.includes("UTF-8") ||
        declaredEncoding.includes("UTF8")
      ) {
        return "UTF-8";
      }
      if (
        declaredEncoding.includes("ISO-8859-1") ||
        declaredEncoding.includes("LATIN-1")
      ) {
        return "ISO-8859-1";
      }
    }
  ...
  private validatePlaceholderIntegrity(
    original: string,
    translated: string,
    ...
    const originalNumbered = originalPlaceholders.filter((p) =>
      /\{[0-9]+\}/.test(p),
    );
    const translatedNumbered = translatedPlaceholders.filter((p) =>
      /\{[0-9]+\}/.test(p),
    );

    if (originalNumbered.length !== translatedNumbered.length) {
      issues.push(
        `Numbered placeholder count mismatch: original has ${originalNumbered.length}, translated has ${translatedNumbered.length}`,
      );
    }
  ...
  private categorizePlaceholders(
    placeholders: string[],

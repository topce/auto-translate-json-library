...
export abstract class BaseFormatHandler implements IFormatHandler {
   ...
   */
  protected mergeValidationResults(
   ...
   */
  private removeDuplicateIssues<T extends { code: string; message: string }>(
    issues: T[],
  ): T[] {
   ...
   */
  protected validateTranslationString(
    value: string,
    path: string,
  ...
  protected validateObjectStructure(
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === "string") {
        const stringValidation = this.validateTranslationString(
          value,
          currentPath,
        );
   ...
   */
  protected validateContentFormat(content: string): {
    errors: any[];
    warnings: any[];
  } {
    const errors: any[] = [];
    ...
    if (!content || content.trim().length === 0) {
      errors.push(
        this.createValidationError("EMPTY_CONTENT", "File content is empty"),
      );
    }

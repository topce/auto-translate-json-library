    ...
    recover: (
      ...
      try {
        ...
        return {
          ...
          warnings: [],
          errors: [
            `Failed to fix trailing commas: ${recoveryError instanceof Error ? recoveryError.message : String(recoveryError)}`,
          ],
          recoveryMethod: "JSON Trailing Comma Fix",
          originalError: error.message,
        };
    ...
    recover: (
      ...
      try {
        ...
        return {
          ...
          data: parsed,
          warnings: [
            "Removed comments from JSON (comments are not standard JSON)",
          ],
          errors: [],
        ...
        return {
          ...
          warnings: [],
          errors: [
            `Failed to remove comments: ${recoveryError instanceof Error ? recoveryError.message : String(recoveryError)}`,
          ],
          recoveryMethod: "JSON Comment Removal",
          originalError: error.message,
        };
...

  {
    ...
    recover: (
      ...
      try {
        ...
        errors.push(
          `Partial recovery failed: ${recoveryError instanceof Error ? recoveryError.message : String(recoveryError)}`,
        );
      }
      ...
      return {
        ...
        errors:
          errors.length > 0
            ? errors
            : ["No valid JSON objects could be recovered"],
        recoveryMethod: "JSON Partial Recovery",
        originalError: error.message,
      };
    },
    ...
    recover: (
      ...
      try {
        ...
        return {
          ...
          warnings: [],
          errors: [
            `Failed to fix XML encoding: ${recoveryError instanceof Error ? recoveryError.message : String(recoveryError)}`,
          ],
          recoveryMethod: "XML Encoding Fix",
          originalError: error.message,
        };
    ...
    recover: (
      ...
      try {
        ...
        while ((match = tagRegex.exec(content)) !== null) {
          ...
          if (isClosing) {
            ...
            if (lastOpenTag !== tagName) {
              warnings.push(
                `Mismatched closing tag: expected ${lastOpenTag}, found ${tagName}`,
              );
            }
          } else {
        ...
        return {
          ...
          warnings: [],
          errors: [
            `Failed to fix unclosed XML tags: ${recoveryError instanceof Error ? recoveryError.message : String(recoveryError)}`,
          ],
          recoveryMethod: "XML Unclosed Tag Fix",
          originalError: error.message,
        };
...
export const formatFallbackStrategies: RecoveryStrategy[] = [
      ...
      try {
        ...
        if (!handler) {
          return {
            ...
            warnings: [],
            errors: [
              `No handler available for detected format: ${detectedFormat}`,
            ],
            recoveryMethod: "Format Auto-Detection Fallback",
            originalError: error.message,
          };
        ...
        try {
          ...
          warnings.push(
            `Successfully parsed using ${detectedFormat} format instead of original format`,
          );
          ...
          return {
            ...
            warnings,
            errors: [
              `Failed to parse with detected format ${detectedFormat}: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
            ],
            recoveryMethod: "Format Auto-Detection Fallback",
            originalError: error.message,
          };
        ...
        return {
          ...
          warnings: [],
          errors: [
            `Format fallback failed: ${recoveryError instanceof Error ? recoveryError.message : String(recoveryError)}`,
          ],
          recoveryMethod: "Format Auto-Detection Fallback",
          originalError: error.message,
        };
...

  {
    ...
    description:
      "Extract translatable strings from any text content as last resort",
    canRecover: (error: Error, content: string) => {
    ...
    recover: (
      ...
      try {
          ...
          if (quotedStrings) {
            for (const quotedString of quotedStrings) {
              ...
              if (cleanString.length > 0) {
                extractedStrings[
                  `line_${i + 1}_string_${Object.keys(extractedStrings).length + 1}`
                ] = cleanString;
              }
            }
          } else if (
        ...
        if (Object.keys(extractedStrings).length > 0) {
          warnings.push(
            `Extracted ${Object.keys(extractedStrings).length} potential translatable strings from plain text`,
          );
          warnings.push(
            "This is a fallback recovery - please verify the extracted content",
          );
        ...
        return {
          ...
          warnings: [],
          errors: [
            `Plain text fallback failed: ${recoveryError instanceof Error ? recoveryError.message : String(recoveryError)}`,
          ],
          recoveryMethod: "Plain Text Fallback",
          originalError: error.message,
        };
...
export class UserGuidanceSystem {
  ...
  static getGuidance(errorType: string): string[] {
    return (
      this.commonIssues.get(errorType) || [

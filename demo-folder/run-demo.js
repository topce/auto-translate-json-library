...
async function ensureHuggingFaceLocalModel(env) {
    ...
    if (!model) {
        throw new Error('ATJ_HUGGING_FACE_LOCAL_MODEL is required for the huggingface-local engine');
    }

    console.log(`\n>>> Preparing Hugging Face local model: ${model}`);
    console.log('    Downloading model from Hugging Face on first run and warming local cache with translation pipeline...');
...
const targetFormat = args.find(arg => arg.startsWith('--format='))?.split('=')[1] || args.find(arg => !arg.startsWith('--'));
const engine = args.find(arg => arg.startsWith('--engine='))?.split('=')[1] || 'huggingface-local';
...
if (args.includes('--help') || args.includes('-h')) {
    ...
    console.log('  node run-demo.js yaml                    # Run YAML folder demo with Hugging Face local');
    console.log('  node run-demo.js --format=po             # Run PO folder demo with Hugging Face local');
    console.log('  node run-demo.js --engine=huggingface    # Run all folder demos with Hugging Face cloud');
    console.log('  node run-demo.js --engine=openai         # Run all demos with OpenAI');
    console.log('  node run-demo.js all --engine=openai     # Run all folder demos with OpenAI');
    console.log('  node run-demo.js                         # Run all folder demos with Hugging Face local');
    process.exit(0);
}
...
console.log(`This script demonstrates the features of the CLI tool using ${engine}.`);
const modelInfo = engine === 'openai' 
